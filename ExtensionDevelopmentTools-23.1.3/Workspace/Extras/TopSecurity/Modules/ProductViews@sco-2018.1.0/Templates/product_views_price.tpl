		...
		{{else}}
			<span class="product-views-price-exact" itemprop="offers" itemscope itemtype="https://schema.org/Offer">
				...
				{{#if showComparePrice}}
					...
					&nbsp;	
					<span class="product-views-price-lead orange" itemprop="price" data-rate="{{price}}">
					{{priceFormatted}}
				</span>
                ...
                <span class="product-views-price-lead_bruto" itemprop=“price” data-rate=“{{price}}“>
                    {{model.pricelevel1_formatted}} <!-- works in one or the other -->
                    {{model.item.pricelevel1_formatted}}
                     {{translate 'bruto'}}
                </span>
